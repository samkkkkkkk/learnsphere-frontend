...
- **프론트 단독 마감 라운드 (2026-07-27)**: #7 공용 Breadcrumb(React 학습·LMS, aria-current), #33 FocusManager 웹캠 모달에 Esc·`role="dialog"`·`aria-modal` 최소 적용(웹캠 로직 미변경), #27 primary 전수 점검 — 유일한 위반이던 로드맵 루트 노드(토글이 대형 앰버 primary)를 아웃라인으로 강등해 "레슨 바로 학습하기" CTA가 두드러지게 함. 체크리스트 ✅ 68개(91%) — 잔여는 백엔드 대기 1건(#16)과 결정 대기 ⚠️ 6건뿐. **프론트 단독으로 진행 가능한 계획 항목 전부 소진.**
